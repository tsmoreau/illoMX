...
contract illomxMARKET is ReentrancyGuard {
  ...
  mapping(uint256 => UserData) private idToUserData;
 ...
 constructor() {
  ...
  function createMarketItem(
    ...
    require((idToMarketItem[tokenId].nftContract != nftContract),
   ...
   function cancelMarketItem(
    ...
    require((idToMarketItem[itemId].nftContract == nftContract),
        "This NFT is already on Sale");
  ...
  function sellOwnedItem(
    ...
    require(idToMarketItem[itemId].nftContract == nftContract);
    require(idToMarketItem[itemId].owner == msg.sender);
    require(idToMarketItem[itemId].onSale == false);
  ...
  function createMarketPurchase(
    address nftContract,
    uint256 itemId
    ) public payable nonReentrant {
    uint price = idToMarketItem[itemId].price;
    uint tokenId = idToMarketItem[itemId].tokenId;
    require(idToMarketItem[itemId].onSale == true);
  ...
  function fetchMyOwnedNFTs(
    ...
    MarketItem[] memory items = new MarketItem[](itemCount);
    for (uint iii = 0; iii < totalItemCount; iii++) {
      if (idToMarketItem[iii + 1].owner == msg.sender) {
        uint currentId = iii + 1;
        MarketItem storage currentItem = idToMarketItem[currentId];
        items[currentIndex] = currentItem;
        currentIndex += 1;
      }
  ...
  function fetchMyMintedNFTs(
    ...
    MarketItem[] memory items = new MarketItem[](itemCount);
    for (uint iii = 0; iii < totalItemCount; iii++) {
      if (idToMarketItem[iii + 1].minter == msg.sender) {
        uint currentId = iii + 1;
        MarketItem storage currentItem = idToMarketItem[currentId];
        items[currentIndex] = currentItem;
        currentIndex += 1;
